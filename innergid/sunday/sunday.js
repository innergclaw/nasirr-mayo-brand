...
const showResult = (claim) => {
  if (claim.claim_status === "claimed") {
    status.textContent = `confirmed. ${claim.membership_number} has full member access through ${formatDateTime(claim.trial_expires_at)}.`;
    action.hidden = true;
    openId.hidden = false;
    remaining.textContent = String(Math.max(claim.capacity - claim.claimed_count, 0));
    return;
  }
  if (claim.claim_status === "trial_used") {
    const active = Date.parse(claim.trial_expires_at) > Date.now();
    status.textContent = active
      ? `your one-time trial is active through ${formatDateTime(claim.trial_expires_at)}.`
      : `your one-time trial ended ${formatDateTime(claim.trial_expires_at)}. choose a membership to restore member access.`;
    action.hidden = true;
    openId.hidden = !claim.membership_number;
    return;
  }
  if (claim.claim_status === "already_has_id") {
    status.textContent = `${claim.membership_number} is already connected to this account. this free trial is for new members.`;
    action.hidden = true;
    openId.hidden = false;
    return;
  }
  if (claim.claim_status === "full") {
    status.textContent = "this week’s five trials are claimed. the next drop opens sunday at midnight et.";
    action.textContent = "this week’s five are claimed";
    action.disabled = true;
    return;
  }
  ...
  if (claim.claim_status === "email_unverified") {
    status.textContent = "confirm your email before a trial place can be assigned.";
    return;
  }
...
const claim = async () => {
  ...
  action.textContent = "checking the five trial places…";
  status.textContent = "verifying your account and trial eligibility…";
  try {
