...
const loadMemberRecord = async (session) => {
  ...
  email.textContent = session.user.email
    ? `Signed in as ${session.user.email}. Your member record is active.`
    : "Your member record is active.";
};

const render = async (session) => {
  if (shouldRedirectToAccount({ session, currentPath: window.location.pathname })) {
    redirecting = true;
    window.location.replace("../account/?next=%2Fdashboard%2F");
    return;
  }
  ...
  setStatus("Loading your INNERG ID...");
  try {
...
nameForm.addEventListener("submit", async (event) => {
  ...
  nameStatus.textContent = "Saving your name...";
  ...
  setMemberCard(data);
  nameStatus.textContent = "Your INNERG ID is ready.";
});
